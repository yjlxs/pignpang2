...
export default {
  ...
  data() {
    return {
      ...
      // 用于生成唯一ID的计数器
      mockIdCounter: 1000, // 从一个大数字开始，避免与真实ID冲突
  ...
  methods: {
    ...
    generateUniqueId() {
      // 使用时间戳 + 随机数 + 计数器确保唯一性
      return Date.now() + '-' + Math.random().toString(36).substr(2, 9) + '-' + (this.mockIdCounter++);
    },
    ...
    async loadTournaments() {
      ...
      try {
        ...
        const total = list.length + 10; // 模拟总数据量
        ...
        const newList = list.filter(newItem => 
          !this.tournaments.some(existingItem => existingItem.id === newItem.id)
        );
        
        this.tournaments.push(...newList)
    ...
    // 下拉刷新
    onRefresh() {
      // 修改：重置mockIdCounter，避免ID无限增长
      this.mockIdCounter = 1000;
      this.finished = false
      this.page = 1
    ...
    viewDetail(id) {
      uni.navigateTo({
        url: `/views/tournament/TournamentDetail?id=${id}`
      })
    },
    ...
    createTournament() {
      uni.navigateTo({
        url: '/views/tournament/TournamentForm'
      })
    }
  }
}
