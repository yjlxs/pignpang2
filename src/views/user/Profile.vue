    ...
    <view class="list-container">
      ...
      <view class="user-card">
        <view class="user-info">
          <image
            class="user-avatar"
            :src="userInfo.avatarUrl || '/static/6328.png_860.png'"
            mode="aspectFill"
          />
...
export default {
  ...
  methods: {
    ...
    async checkUserInfo() {
      try {
        ...
        if (storedUserInfo) {
          ...
          try {
            // 使用API获取完整的用户信息
            // 注意：这里需要你根据实际项目调整API调用
            // const response = await this.$api.user.getUserCompleteInfo(parsedUserInfo.id)
            ...
            this.hasAdminRole = this.checkAdminRole(parsedUserInfo.roles)
          } catch (error) {
    ...
    goToUserDetail() {
      uni.navigateTo({
        url: '/views/user/UserDetail'
      })
    },
    
    goToMyEvents() {
      uni.navigateTo({
        url: '/views/match/MatchHistory'
      })
    },
